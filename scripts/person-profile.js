...
function personGetEventPageUrl(eventKey) {
  return `repertoire/${eventKey}.html`;
}
...
function personCreateFact(iconPath, label, value) {
  ...
  valueElement.style.whiteSpace = "pre-line";
...
function personGetPerformances(personId) {
  const events = window.EVENTS_DATA || {};

  return Object.entries(events).flatMap(
    ([eventKey, event]) => {
...
function personCreatePerformanceCard(performance, events) {
  ...
  const eventUrl = personGetEventPageUrl(
    performance.eventKey
  );
  ...
  imageWrap.append(
    personCreatePicture({
      source: personGetEventImagePath(
        performance.eventKey
      ),
...
function personRenderPerformances(personId, root) {
  if (!root) {
    return;
  }

  const performances = personGetPerformances(personId);
  const events = window.EVENTS_DATA || {};
...
function initPersonProfile() {
  ...
  if (type === "troupe") {
    ...
    personRenderPerformances(personId, performancesRoot);
